...
from design_calcs import (
    LoadInputs, ZSectionInputs, PURLIN_END_BAY_COEFF, PURLIN_MID_BAY_COEFF,
    purlin_loads, purlin_moments, z_section_properties, code_checks
)
...
checks = code_checks(inp, sec, props, moments["gravity_support_moment_kg_m"], moments["gravity_span_moment_kg_m"])

st.divider()
load_cols = st.columns(4)
load_cols[0].metric("Kx", f"{loads['kx']:.4f}")
load_cols[1].metric("Gravity load", f"{loads['gravity_load_kg_m']:.2f} kg/m", loads["gravity_direction"])
load_cols[2].metric("Wind net load", f"{loads['wind_net_load_kg_m']:.2f} kg/m", loads["wind_direction"])
...
check_df = pd.DataFrame([
    ...
    {"Check": "Minimum depth rule", "Limit/Value": f"dmin = {checks['minimum_depth_required_mm']:.1f} mm", "Status": "OK" if checks['minimum_depth_check_ok'] else "REVIEW"},
    {"Check": "Support bending stress", "Limit/Value": f"{checks['support_actual_stress_n_mm2']:.2f} ≤ {checks['basic_design_stress_n_mm2']:.2f} N/mm²", "Status": "OK" if checks['support_stress_ok'] else "NOT OK"},
    {"Check": "Span bending stress", "Limit/Value": f"{checks['span_actual_stress_n_mm2']:.2f} ≤ {checks['basic_design_stress_n_mm2']:.2f} N/mm²", "Status": "OK" if checks['span_stress_ok'] else "NOT OK"},
])
st.dataframe(check_df, use_container_width=True, hide_index=True)

st.warning("Verify final design against IS 801 and project/client criteria before issue. This app is a reusable calculation scaffold extracted from the workbook.")
